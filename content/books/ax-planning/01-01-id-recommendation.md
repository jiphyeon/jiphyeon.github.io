...
![사용자가 이름과 이메일을 입력하면 세 가지 방식으로 분기되어 아이디 후보를 생성하고, 공통 정제 단계를 거쳐 최종 추천이 이루어지는 프로세스 흐름도. 세 가지 방식은 각각 자연어 처리 기반(분석 및 토큰화 → 연관 키워드 생성 → 단순 변형 및 조합), 데이터 기반(기존 유사 아이디 패턴 추출 → 트렌드 키워드 분석 → 유사 패턴 기반 추천), 생성형 AI 기반(벡터 의미 분석 및 추출 → 창의적 아이디 생성 → 독창적 아이디 추천)으로 구성되며, 세 경로가 합쳐진 후 규칙 기반 정제 → 감성 분석 → 서비스 특성 반영 순서로 최종 정제가 이루어짐.](/images/docs/ax-planning/01-01-id-recommendation.png)
